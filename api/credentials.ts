...
import { validateFeaturePayloadStrict, CreateCredentialRequestSchema } from "../server/validation/requests";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const appOrigin =
      ...
      (req.headers.host ? `${req.headers["x-forwarded-proto"] || "http"}://${req.headers.host}` : "http://localhost:5173");
