...
export function resizeToCanvas(
	...
	maxEdge: number = MATCH_CONFIG.TARGET_LONG_EDGE,
): HTMLCanvasElement {
...
export async function extractOrbFeatures(
	...
	try {
		...
		if (numPoints < MATCH_CONFIG.MIN_KEYPOINTS_CLIENT) {
			throw new Error(
				`图片纹理不足（仅检测到 ${numPoints} 个特征点，需要至少 ${MATCH_CONFIG.MIN_KEYPOINTS_CLIENT} 个），请换一张纹理丰富、清晰的图片。`,
			)
		}
		...
		if (
			...
			descriptors.cols !== MATCH_CONFIG.DESCRIPTOR_SIZE
		) {
